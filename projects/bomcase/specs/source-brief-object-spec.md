...
v0.3 同时明确：事实定义层由产品规则定义和表达定义共同构成；A:view 是验收检查视图，不属于事实定义层。

### 8.2 Layer Definitions

v0.3 区分“事实定义层”和“验收检查视图”。F / E / F+E 用于标注产品定义的读取入口；A:view 是基于产品定义生成的检查视图，不属于事实定义层。

Fact Definition Layer 是产品定义总称，不等同于 `section_layer` 的 F。
在 `section_layer` 标注体系中：

* 规则类定义归 F；
* 表达类定义归 E；
* 规则与表达混写归 F+E；
* A:view 是检查视图，不是事实定义层。

| Layer | Name | Definition | Deletion Impact |
|---|---|---|---|
| F | Fact / 产品事实层 | 定义产品结构、字段、规则、计算、触发条件、业务边界、跨页关系、pending / decision 承接。 | 删除后会改变产品定义。 |
| E | Expression / 表达层 | 定义用户可见文案、展示状态、视觉表达、hover / info / 按钮 / 状态 / 汇总文案、区域内小粒度呈现。 | 删除后不改变规则定义，但会影响产品表达定义与 UI 表达完整性。 |
...
* Fact Definition Layer 是产品定义总称，不等同于 `section_layer` 的 F。
* F 包含主规则、状态规则、交互规则、边界规则；状态触发、交互触发、计算、条件、结果、边界按 F 读取；F 不包含用户可见文案本身。
* E 是产品定义中的表达定义层，不是视觉装饰；E 包含用户可见文案、按钮文案、info / hover 文案、轻提示文案、状态文案、标签和展示表达；E 在标注体系中不按 F 读取。
* F+E 用于规则与表达混写章节，典型场景包括规则 + 文案、状态触发 + 状态文案、交互结果 + 反馈文案、边界规则 + 用户提示、字段含义 + 展示文案；F+E 章节读取时必须局部拆分，不能整体当作 F 或 E。
* A:view 是基于 F / E / F+E 派生的验收检查视图，不是事实源，不属于 Fact Definition Layer，不参与 Main Fact Anchor 选择，不作为 Delivery Output 正文主来源。
...
`A:view` 是由 F / E / F+E 派生的验收检查视图，不属于 Fact Definition Layer，不参与 Main Fact Anchor 选择，只能用于 coverage check、测试验收和产品人工验收。`A:view` 不得作为 Delivery Output 正文主来源。
...
`F+E` 的拆分规则：状态触发、展示条件、交互触发、计算结果、边界约束按 F 读取；用户可见文案、按钮文案、状态文案、info / hover 文案、轻提示文案按 E 读取。
...
For Delivery Output handoff, A:view is coverage check only; do not expand into delivery 正文; do not use as primary fact source.
...
* A:view 对 F / E 的复述不构成事实冗余，但也不得驱动 Delivery Output 重复展开。
* Delivery Output 应从 F / E / F+E 读取正文内容，并使用 A:view 检查覆盖。
