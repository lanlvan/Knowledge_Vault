...
### Generation Report / Delivery Body Separation Rule

Handoff delivery body refers only to the 13-section skeleton body defined in `Recommended Full Handoff Structure`.

`Delivery Output Generation Report` is a generation report and self-check report. It is not part of the development delivery body.

Generation Report must not be mixed into sections 1-13 of the handoff body.

Coverage Baseline, `N_e` / `N_f`, Copy Coverage Failure, Sources Read, Information Layer Check, and other governance or generation-control content must not enter the development delivery body.

Test drafts may append Generation Report after the delivery body, but the report must be clearly separated from the body.

When the user command explicitly includes any of the following output requirements, the handoff file body must not include Generation Report:

* clean handoff
* formal delivery draft
* delivery-ready
* 正式交付正文
* 正式交付稿
* 清洁版
* 纯净版
* remove Generation Report
* no Generation Report
* 去除 Generation Report
* 去掉 Generation Report
* 不包含 Generation Report
* 不输出 Generation Report
* only output 研发交付正文
* 只输出交付正文
* 只输出研发交付正文
* 只输出研发交接正文

In the above scenarios, the mandatory Generation Report mechanism is not cancelled. Generation Report may be returned as the assistant response or as an independent governance report.

This rule does not create a new clean, formal, delivery-ready, or dev-readable output mode.
...
### Density as Residual Rule

Density is not an independent optimization target for Full Mode.

Character count, document length, and short-text ratio must not be used as pass / fail gates.

Do not remove source fact items to reduce word count, shorten the document, or increase density.

Readability optimization may only be achieved by reducing repeated explanation, referencing delivery-body ids, removing governance language from the body, and improving structure order.

Fields, states, interactions, copy, boundaries, and acceptance items remain protected by Full Mode Completeness Floor.

Whether an output feels too dense or too loose may be used only as a human reading observation after Copy Coverage passes. It must not override Copy Coverage judgment.

Do not rewrite Density as Residual Rule into a new compression objective.

### Full Mode Completeness Floor

Full Mode readability compression has a completeness floor.

Compression may compress repeated explanations, repeated role-boundary wording, duplicated background, and repeated visual implementation caveats. Compression must not compress, remove, or downgrade source fact items themselves.

In Full Mode, the following source content must not be downgraded into area-level summary, module-level summary, or directional wording:
...
* service-return boundaries;
* display / non-display conditions.

If active source expresses key content as a table or list, Delivery Output may reorganize it, merge duplicate explanatory notes, or move rows to a more suitable delivery section, but each source item must remain traceable.

Traceable means the output item retains enough condition, copy, result, boundary, or non-applicable handling for implementation, UI comparison, or test validation.

Pending / supplement items must not be omitted and must not merge different questions into one generic issue. Explanatory background may be compressed, but each pending / supplement question must remain an independent traceable item.

Repeated explanation may be merged only when item traceability is preserved.

Coverage must use four levels:

| Level | Definition | Counts as covered |
|---|---|---|
| Covered | The item is traceable, implementable, and testable in the output. | Yes |
| Partial | The direction exists, but row-level content, condition, copy, or result is incomplete. | No |
| Mentioned Only | Only a title, area summary, module summary, or generic mention exists. | No |
| Missing | The item is absent from the output. | No |

Only Covered items count toward coverage rate. Partial and Mentioned Only are coverage gaps and must not be counted as Covered.

For each key content group:

```text
coverage_rate = Covered item count / active source item count
```

The active source item count must be counted from the current active source during generation. Do not reuse prior validation, dry run, baseline, historical output, manual estimate, or external report numbers as the denominator.

The active source count is the only valid denominator for coverage rate.

If coverage_rate is lower than 80% for any key content group, Copy Coverage fails.

If any critical item is Missing, Copy Coverage fails even when the overall coverage rate is 80% or higher.

Critical items include user-visible copy, state copy, bottom summary copy, empty-state copy, state transition condition, interaction trigger and result, display / non-display condition, service-return boundary, pending / supplement item, non-goal item, and key acceptance boundary.
...
### F+E Local Split Completeness Guard

F+E local split must preserve completeness after splitting and must not weaken the existing F+E Local Split Execution Rule.

After splitting an F+E source section:

* E-layer user-visible copy must be placed in the corresponding copy table, state copy table, button / entry copy table, hint / hover / info copy location, empty-state table, or expression boundary section.
* F-layer triggers, display conditions, state transition conditions, interaction conditions, service-return boundaries, and business constraints must be placed in the corresponding rule table, state rule, interaction rule, boundary section, or acceptance boundary.
* A source F+E table must not be compressed into a principle statement, area description, or generic summary.
* If a source F+E section contains fields, states, copy, and interactions together, Delivery Output must split them into the corresponding delivery sections while preserving traceable items.

Use the following counts for F+E completeness checking:

```text
N_e_source = count of E-layer user-visible expression items in current active source F+E sections.
N_e_output = count of those E-layer items that are Covered in the output.

N_f_source = count of F-layer trigger, display condition, state transition condition, interaction condition, service-return boundary, or business rule items in current active source F+E sections.
N_f_output = count of those F-layer items that are Covered in the output.
```

`N_e_source` and `N_f_source` must be counted from the current active source during generation. Do not use prior validation, dry run, baseline, historical output, manual estimate, or external report numbers.

If counting depends on specific source sections or line ranges, the Generation Report must identify the source section. If line numbers are available, it must also identify the line range.

Counting rules:

* Count one E item for each distinct user-visible copy, button copy, entry copy, state copy, result copy, hint copy, hover / info copy, bottom summary copy, empty-state copy, label, or user-visible expression requirement.
* If one source row contains multiple distinct user-visible alternatives for different states, count each alternative separately.
* Count one F item for each distinct trigger condition, display condition, non-display condition, state transition condition, interaction condition, result rule, service-return boundary, business boundary, or non-applicable handling.
* If one source row contains both a condition and a result boundary, count them as separate F items when they are independently implementable or testable.
* Repeated wording that points to the same requirement may be deduplicated for counting, but row-level distinct scenarios must not be collapsed.
* Estimated count is acceptable only when the source structure is not fully normalized; the Generation Report must state the counting basis.
...
If `E_coverage_rate` or `F_coverage_rate` is lower than 80%, Copy Coverage fails.

If any critical F+E item is Missing, Copy Coverage fails even when both coverage rates are 80% or higher.

Partial and Mentioned Only must not be counted in `N_e_output` or `N_f_output`.
...
### Region-Aligned Internal Structure Rule

Role-based reading paths should be front-loaded and concise.

Within the fixed 13-section skeleton, fields, states, interactions, and acceptance should preferentially be organized by the page-region order from the current active source.

The page-region order must come from the current active source or current page brief.

When generating a handoff, the generator must read the page structure order from the current active source and page brief.

Historical drafts, loop outputs, P0 / P1 / P2 outputs, and model memory must not be used as the source of page-region order.

The same module should use consistent naming across field, state, interaction, and acceptance sections.

The same module's fields, states, interactions, and acceptance items should remain mutually locatable where practical.

Do not add first-level sections for region alignment.

Do not rewrite fact ownership for structural uniformity.

Do not forcibly add modules or section content that does not exist in the source.

If the page complexity does not support detailed regional grouping, a lighter regional organization is allowed.
...
### Referenced Acceptance Rule

Body rules must remain fully expressed in the field, terminology, state, interaction, and boundary sections.

Acceptance sections must not reduce repetition by deleting acceptance points.

Acceptance sections should preferentially reference body sections, table ids, state ids, rule ids, or interaction ids when the referenced body content already fully states the rule.

Each acceptance item must remain checkable and testable.
...
Referenced acceptance must not weaken A:view coverage and must not fall below Full Mode Completeness Floor.
...
### A:view Reverse Protection / Acceptance Boundary Floor

A:view is an acceptance/check view, not a replacement layer for F / E / F+E.

F / E / F+E are the delivery fact body.

If the same fact, copy, condition, boundary, or acceptance point appears both in A:view and in F / E / F+E, Delivery Output must still preserve the F / E / F+E item at the required delivery granularity.

A:view Handling Rule limits how A:view sections are expanded. It must not propagate backward to reduce field, state, interaction, copy, rule, boundary, pending, non-goal, or other fact-body coverage from F / E / F+E sections.

For normal Full Mode handoff, A:view must not be expanded into a full QA checklist unless the requested output is QA / Acceptance Draft.

However, Full Mode must preserve an acceptance boundary floor sufficient to validate:

* field display;
* state recognition;
* state copy;
* interaction trigger and result;
* user-visible copy;
* empty-state behavior;
* service-return boundary;
* display / non-display condition;
* pending / supplement boundary;
* non-goal / excluded scope.

Acceptance items may be consolidated when they duplicate field, state, or interaction sections, but consolidation must not remove required validation dimensions or reduce them to directional wording.
...
For Full Mode, include a coverage baseline comparison between the current active source and the current output.

Use only the current active source and the current generated output. Do not use prior validation, dry run, baseline, historical output, old output, draft output, archived output, manual estimate, external report, or loop output as the source count denominator.

The active source count is the only valid denominator for coverage rate and must be counted during generation.

Do not treat a shorter output as a positive metric. The only positive metric is sufficient traceable coverage with acceptable readability.

For each source count, state the counting basis. If the count comes from specific source sections or line ranges, list the source section name. If line numbers are available, list the line range.

Estimated count is acceptable only when the source structure requires judgment, and the report must state the estimate basis.

Coverage judgment must use four levels:

| Level | Definition | Counts as covered |
|---|---|---|
| Covered | Item is traceable, implementable, and testable. | Yes |
| Partial | Direction exists, but item-level condition, copy, result, or boundary is incomplete. | No |
| Mentioned Only | Only title, area summary, module summary, or generic mention exists. | No |
| Missing | Item is absent. | No |

Only Covered items count toward output count and coverage rate. Partial and Mentioned Only must not be counted as Covered.
...
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| State copy items |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| State transition rules |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Field items |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Interaction items |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Empty-state copy |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Acceptance boundaries |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Pending / supplement items |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| Non-goal items |  |  |  |  |  |  |  | Yes / No | Pass / Fail |
| F+E E-layer items (`N_e`) | `N_e_source` | `N_e_output` |  |  |  |  |  | Yes / No | Pass / Fail |
| F+E F-layer items (`N_f`) | `N_f_source` | `N_f_output` |  |  |  |  |  | Yes / No | Pass / Fail |
...
Copy Coverage Failure must be marked if any of the following is true:

* any required content group has coverage rate lower than 80%;
* `E_coverage_rate < 80%`;
* `F_coverage_rate < 80%`;
* any critical item is Missing;
* any state table, field table, interaction table, or acceptance boundary is reduced to area-level summary;
* Partial or Mentioned Only affects a critical item required for implementation, UI comparison, or test validation.

If Copy Coverage Failure is marked, the output must not claim to be a complete handoff. It must output Copy Coverage Failure Report instead of a completed handoff.
